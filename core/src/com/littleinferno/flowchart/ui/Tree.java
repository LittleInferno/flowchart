...
public class Tree extends WidgetGroup {
    ...
    private void computeSize(Array<Node> nodes, float indent) {
        ...
        for (int i = 0, n = nodes.size; i < n; i++) {
            ...
            if (node.expanded) computeSize(node.children, indent);
        }
    }
    ...
    private float layout(Array<Node> nodes, float indent, float y) {
        ...
        for (int i = 0, n = nodes.size; i < n; i++) {
            ...
            y -= ySpacing;
            if (node.expanded) y = layout(node.children, indent + 0/*indentSpacing*/, y);
        }
        return y;
    }
    ...
    private void draw(Batch batch, Array<Node> nodes, float indent) {
        ...
        for (int i = 0, n = nodes.size; i < n; i++) {
            ...
            if (node.expanded) draw(batch, node.children, indent);
        }
    }
